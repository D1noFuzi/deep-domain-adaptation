...
def estimator_model_fn(features, labels, mode, params):
    ...
    if FLAGS.step == 'source':
        ...
        input_layer_source = tf.reshape(input_layer_source, [batch_size, 32, 32, params['channel_size']])
    ...
    if FLAGS.step == 'target':
        ...
        # CNNs need input data to be of shape [batch_size, width, height, channel]
        input_layer_source = tf.reshape(input_layer_source, [batch_size, 32, 32, params['channel_size']])
        input_layer_target = tf.reshape(input_layer_target, [batch_size, 28, 28, params['channel_size']])
        # Resize SVHN source to 28 x 28
        input_layer_source = tf.image.resize_images(input_layer_source, [28, 28])
        ...
        tf.train.init_from_checkpoint(tf.train.latest_checkpoint('./model_s2m/source_model/'),
...
def main(_):
    ...
    if FLAGS.step == 'source':
        ...
        (x_train, y_train), (x_test, y_test) = dp.load_svhn(channel_size=1, truncate=False)
        # Configurations first
        iter_ratio = math.ceil((x_train.shape[0] / FLAGS.batch_size))
        print(iter_ratio)
        # SVHN image shape is 32x32x3
        feature_columns = [tf.feature_column.numeric_column("source", shape=(32, 32, FLAGS.channel_size))]
    ...
    if FLAGS.step == 'target':
        # Load SVHN dataset
        (x_train_s, y_train_s), (x_test_s, y_test_s) = dp.load_svhn(channel_size=1)
        # Load MNIST dataset
        (x_train_t, y_train_t), (x_test_t, y_test_t) = dp.load_mnist(channel_size=1)
        # Configurations first
        ...
        feature_columns = [tf.feature_column.numeric_column("source", shape=(32, 32, FLAGS.channel_size)),
                           tf.feature_column.numeric_column("target", shape=(28, 28, FLAGS.channel_size))]
